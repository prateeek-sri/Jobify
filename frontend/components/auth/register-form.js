...
export function RegisterForm() {
  ...
  const customGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        if (res.ok) {
          ...
          showProfessionalToast("Signed up with Google!");
          router.push("/dashboard");
        } else {
          showProfessionalToast(data.msg || "Google signup failed");
    ...
    onError: () => {
      showProfessionalToast("Google Signup Failed");
      setError("Google Signup Failed");
    }
  });

  const handleGoogleSuccess = async (credentialResponse) => {
    showProfessionalToast("Use login page for Google sign in.");
    router.push("/login");
  };
      ...
      <form onSubmit={handleSubmit} className="w-full">
        <div className="space-y-4 mb-6">
          <div className="flex flex-col">
            <label className="text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1" htmlFor="name">Full Name</label>
            <input
              id="name"
              type="text"
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-[#1e1e1e] dark:bg-white hover:bg-black dark:hover:bg-zinc-200 text-white dark:text-black font-medium py-3 px-4 rounded-md transition-colors flex items-center justify-center mb-8 shadow-sm"
        >
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Already a member?{" "}
          <Link href="/login" className="text-black dark:text-white font-bold border-b border-black dark:border-white pb-[1px] hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
            Sign in
          </Link>
        </p>
      </form>
